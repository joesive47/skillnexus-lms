...
if ($TargetDatabase -ne $DB_NAME) {
    Write-Error "TargetDatabase must exactly match the database in DATABASE_URL."
    exit 1
}

if (-not $AllowProductionRestore -and $DB_NAME -notmatch '(restore|drill|test|_runall)$') {
    Write-Error "Restore is allowed only to an isolated restore/drill/test/_runall database. Use -AllowProductionRestore only during an approved incident."
    exit 1
}
...
$CONFIRM = Read-Host "Type the exact database name '$DB_NAME' to continue"

if ($CONFIRM -ne $DB_NAME) {
