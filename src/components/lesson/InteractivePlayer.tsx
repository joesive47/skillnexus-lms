...
export default function InteractivePlayer({ 
  ...
  useEffect(() => {
    const handleMessage = async (event: MessageEvent) => {
      const iframe = document.getElementById('interactive-frame') as HTMLIFrameElement | null
      if (event.source !== iframe?.contentWindow) return
      if (event.data?.score !== undefined) {
        const { score, passed } = event.data
        if (typeof score !== 'number' || !Number.isFinite(score) || score < 0 || score > 100 || typeof passed !== 'boolean') return
