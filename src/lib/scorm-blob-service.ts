...
export class ScormService {
  ...
  async uploadPackage(file: File, lessonId: string, replace: boolean = false): Promise<string> {
    try {
      ...
      // Validate the archive and manifest before creating a public blob.
      const manifest = await this.parseManifestFromFile(file)
      if (!manifest) throw new Error('A valid imsmanifest.xml is required')
  ...
  private async parseManifestFromFile(file: File): Promise<ScormManifest | null> {
    try {
      if (file.size <= 0 || file.size > 50 * 1024 * 1024) {
        throw new Error('SCORM package must be between 1 byte and 50 MB')
      }
      const buffer = Buffer.from(await file.arrayBuffer())
      if (buffer.length < 4 || buffer[0] !== 0x50 || buffer[1] !== 0x4b) {
        throw new Error('SCORM package is not a ZIP archive')
      }
        ...
        yauzl.fromBuffer(buffer, { lazyEntries: true }, (err, zipfile) => {
          ...
          zipfile.on('entry', (entry: yauzl.Entry) => {
            ...
            if (entryCount > maxEntries || expandedBytes > maxExpandedBytes) {
              zipfile.close()
              reject(new Error('SCORM package expands beyond the allowed limits'))
              return
            }
            ...
            if (entry.fileName.toLowerCase().includes('imsmanifest.xml')) {
              if (entry.uncompressedSize > maxManifestBytes) {
                zipfile.close()
                reject(new Error('SCORM manifest is too large'))
                return
              }
              manifestFound = true
